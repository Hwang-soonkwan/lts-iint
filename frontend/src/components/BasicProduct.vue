<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            제품마스터 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            제품마스터
        </v-card-title >        
        ...
        <v-card-text>
            <Number v-if="editMode" label="제품고유번호" v-model="value.prodNo" :editMode="editMode" :inputUI="''"/>
